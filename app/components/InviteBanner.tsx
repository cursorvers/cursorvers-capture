...
export function InviteBanner(): JSX.Element | null {
  ...
  if (phase === "claiming") {
    return (
      <div className="mx-auto mt-3 max-w-md rounded-2xl border border-accent/40 bg-accent/10 px-4 py-3 text-[13px] text-accent-soft">
        🎟️ 招待を受け取り中…
      </div>
    );
  }
  if (phase === "success") {
    return (
      <div className="mx-auto mt-3 max-w-md rounded-2xl border border-emerald-400/40 bg-emerald-400/10 px-4 py-3 text-[13px] text-emerald-200">
  ...
  if (phase === "error") {
    return (
      <div className="mx-auto mt-3 max-w-md rounded-2xl border border-red-400/40 bg-red-400/10 px-4 py-3 text-[13px] text-red-200">
