...
- 将来、画像内のテキスト読み取り (OCR) や音声要約、相談チャット等の
  追加機能を有効化された場合に限り、対象データが当社が運営する
  別サーバー（社内名称: Codex App Server）経由で処理されます。
  現時点ではこれらの機能は **未稼働** で、画像の流れは
  「端末 → Google Drive のみ」です
- 保存データの削除は、Google アカウント側でのアプリ権限解除、本アプリ
  設定からのサインアウト、または端末ローカルデータの消去によりいつでも
  ご自身で行っていただけます (Drive 内のファイル削除はご自身のアカウント
  操作によります)

【ご利用範囲のお願い】
- 本アプリは記録・撮影補助のための実験的ツールであり、**臨床診断・診療
  判断・医療行為そのものへの利用は意図していません**。詳細は利用規約の
  「AI 機能とモデレーション」項をご確認ください
...
なお本アプリは記録・撮影補助のための実験的ツールであり、**臨床診断・
診療判断・医療行為そのものへの利用は意図していません** (詳細は利用規約の
「AI 機能とモデレーション」項)。
