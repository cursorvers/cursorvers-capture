...
- アクセス権限は「自分が新規作成したファイル」のみに限定されており、
  既存ファイルの読み取りは行いません (Google が定義する Drive 用
  アクセス権限の中で、ファイル本体の読み書きに関わるもっとも制限的な
  権限の一つ、`drive.file` スコープを使っています)
...
【ご自身で検証いただける項目 (任意)】
- ブラウザの「開発者ツール」(Chrome なら右上 ⋮ → その他のツール →
  デベロッパーツール) → Network タブを開いた状態で撮影 → upload を
  行うと、画像の送信先 URL が `googleapis.com`(Google の Drive API) のみ
  であり、当社のドメインへ画像本体が送られていないことをご自身で確認
  いただけます
...
## 送信後フォローアップフロー (sublimation: 顧客成功の起点化)

配布メール A/B 送信後の標準フォロー (critic 合議で blind_spot として指摘):

| 経過 | アクション | 判断基準 |
|---|---|---|
| 送信当日 | 開封確認 (Gmail 既読は確認不可、間接シグナルは「数時間以内に返信があるか」) | 翌日まで様子見 |
| **3 日後** | ご試用状況をやんわり確認するリマインド (短文 1 通)。例: 「先日ご案内したアプリ、もしお時間に余裕がございましたらお試しいただけますと幸いです。お忙しい場合は気にされませんよう」 | 3 日経って初回 upload も返信もなければ送る (押し付けない) |
| **1 週間後** | 1 回でも upload があれば「使い心地のフィードバック依頼」、なければ無理に push せず Q3 持ち越し判断 | OPERATIONS.md §7 Exit 条件の deadline (香西氏 6/5、長谷川氏 6/2) と整合 |
| **3 週間後 (deadline 到達)** | OPERATIONS.md §7 の deadline 到達。Exit 条件を満たせなければ「配布見送り / Q3 持ち越し」として close | 永続検証は user 時間を蝕む |

### リマインドメール文面例 (3 日後、A/B 共通)

```
件名: Re: [Cursorvers] 写真アップローダー先行試用のご案内

<宛名>

先日ご案内した写真アップローダーの件、その後いかがでしょうか。
お忙しい中恐縮ですが、もしお時間に余裕がございましたらお試しいただけますと
幸いです。もし当面お試しいただく時間が取れないようでしたら、その旨ひと言
お知らせいただければ次回のタイミングを改めて伺います。
